/* eslint-disable prefer-template, quote-props, quotes, max-lines, no-console */
/* eslint-disable no-restricted-syntax, guard-for-in, prefer-arrow-callback, semi, key-spacing */
/* eslint-disable no-var, comma-style, comma-spacing, no-trailing-spaces */
/* eslint-disable vars-on-top, prefer-destructuring, no-plusplus */
/* eslint-disable no-lonely-if */

// === Phantom Fort/Fortress Game ===
function PFGameClass() {
	...
	*/
	...
	this.loop = function loop() {
		...
		if (o.isLooping) {
			...
			if (o.loopIteration < 15000000) {
				o.loopTimer = window.setTimeout(() => { o.loop(); }, o.loopDelay);
			} else {
	...
	this.drawTower = function drawTower(isAnimated) {
	...
	this.populateTower = function () {
		...
		for (var f in this.game.floorArray) {
			...
			for (const i in floor.goonKeyArray) {
			...
			for (const i in floor.invaderKeyArray) {
	...
	this.setToon$Elt = function setToon(toon, toonType) {
		// console.log(toon);
	...
	this.removeSpaces = function removeSpaces(t) {
		return t.replace(/\s+/g, ''); // Remove spaces
	}

	this.calculateFloorsCounts = function calculateFloorsCounts() {
		var foundGroundFloor = false;
		let t = 0;
		let b = 0;
		let fIndex = 0;
	...
	this.deselectFloor = function deselectFloor() { // (floorKey) {
		this.selectedFloorKey = "";
	...
	this.viewFloor = function (floorKey, top, left) {
		...
		window.$('.workerCount').html(workerCount + "/" + floorTypeObj.workerSpaces);
		...
		if (!floorTypeObj.workerSpaces || floorTypeObj.workerSpaces === 0) {
		...
		console.log("floorObj.naturalResources", floorObj.naturalResources);
		for (var n in floorObj.naturalResources) {
	...
	this.viewFloorPurchase = function (top) {
		...
		for (var floorTypeId in o.data.floors) {
			...
			if (!floorType.forSale) {
				...
				$li.click(function liClick(e) {
					var $this = window.$(this);
					const clickedFloorTypeId = $this.data("floortypeid");
					// var isTop = $this.data("istop");
					o.buyFloor(floorKey, clickedFloorTypeId);
					e.preventDefault();
				});
	...
	this.viewBuilder = function (isTop, top, left) {
		...
		o.$floorBuilder.find('.buildNewFloor').off("click").on("click", function () {
	...
	this.getFloorCost = function (floorType, isNewFloor, isTop) {
		...
		// Not building a new floor, just remodeling
			finalCost = structuredClone(floorType.cost);
	...
	this.isFloorOnTop = function (floorKey) {
		...
		for (var i = 0; i < floorNum; i++) {
			const iterativeFloorKey = this.game.floorArray[i];
			if (iterativeFloorKey === floorKey) {
				return isTop;
			}
			if (iterativeFloorKey === "F0") {
				isTop = false;
	...
	this.buyGoon = function (goonTypeId, floorKeyParam) {
		const floorKey = floorKeyParam || this.selectedFloorKey;
		const o = this;
		if (o.isRoomForWorkers(floorKey)) {
			var goonTypeObj = o.data.goons[goonTypeId];
			var canAfford = true;
			for (const currency in goonTypeObj.cost) {
	...
	this.addGoon = function (goonTypeId, floorKeyParam) {
	...
	this.addInvader = function (invaderTypeIdParam) {
		...
		const invaderTypeId = (typeof invaderTypeIdParam === 'number')
			? invaderTypeIdParam
			: this.roll1d(this.data.invaders.length) - 1;
		...
		const startFloor = 'F0';
		this.game.floors[startFloor].invaderKeyArray.push(newKey);
		this.populateTower();
	...
	this.hitToon = function (toon, hittingToon) {
		...
		if (toon.x < hittingToon.x) {
			toon.externalVelX -= (8 / damper);
		} else if (toon.x > hittingToon.x) {
			toon.externalVelX += (8 / damper);
		} else {
			toon.externalVelY += (1 / damper);
		}
	}
	...
	this.updateToonFacing = function (toon) {
		...
		var isLeftFacing = Boolean(toonEltClassList.contains("leftFacing"));
	...
	this.doCombatRound = function doCombatRound() {
		const o = this;
	...
	this.damageToon = function damageToon(toon, damageParam = 0) {
		const damage = damageParam + this.roll1d(2) - 1; // bonus random damage
		toon.hp -= damage;
		var $dmg = window.$('<div class="damage">' + damage + '</div>');
	...
	this.removeToonFromFloor = function (toon, toonKey, callback) {
		...
		var keyPos = window.$.inArray(toonKey, floor[arrayName]);
	...
	this.addToonToFloor = function (toon, toonKey, newFloorKey) {
		...
		if (toon.isGoon) {
			arrayName = "goonKeyArray";
			toonType = this.data.goons[toon.goonTypeId];
		} else {
			arrayName = "invaderKeyArray";
			toonType = this.data.invaders[toon.invaderTypeId];
		}
	...
	this.calculatePerSecondValues = function () {
		...
		for (fIndex in this.game.floorArray) {
			...
			var canProduce = true;
			var canHarvest = (floorType.name === "Mine");
			...
			if (canHarvest && numOfWorkers > 0 && typeof floor.naturalResources === "object") {
				...
				for (const currency in floor.naturalResources) {
			...
			if (numOfWorkers > 0) {
				...
				if (typeof floorType.input === "object") {
					for (const currency in floorType.input) {
	...
	*/
	...
	this.getDisplayNumber = function (nParam) {
	...
	this.setZoom = function (zoomLevel) {
		...
		if (zoomLevel < 0) return false; // zoomLevel = 0;
		this.zoom = zoomLevel;
		this.$tower.add('body') // .data("zoom", zoomLevel)
			.removeClass("zoom0").removeClass("zoom1").removeClass("zoom2")
			.removeClass("zoom3")
	...
	this.setup = function () {
		...
		$.ajax({
			...
			,dataType: "json"
			,success: function success(responseObj) {
				try {
			...
			,error: function error(x, textStatus, errorThrown) {
		...
		*/
		...
		$('.stopLoop').click(() => { 	o.stopLoop(); });
		$('.startLoop').click(() => { 	o.startLoop(); });
	...
	this.saveGame = function saveGame(showNotice) {
	...
	this.deleteGame = function deleteGame() {
		localStorage.removeItem("owned");
		localStorage.removeItem("total");
		this.notify("Saved game deleted!");
	}
	
	this.loadGame = function loadGame(isStartLoop) {
		const { $ } = window;
		const o = this;
		// var isLoaded = false;
	...
	this.toggleSound = function (forceSound) {
		...
		else this.isSoundOn = !this.isSoundOn;
	...
	*/
	...
	this.playSound = function (soundNameParam, isLooped) {
		let soundName = soundNameParam;
		if (this.isSoundOn) {
